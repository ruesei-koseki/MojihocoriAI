...
def looking(x, u, reply=True, force=False):
    ...
    try:
        ...
        # Similar alternating pattern for user-prefixed sentences
        rate = 1
        while rate > 0:
            rate -= a
            max_offset = max(DATA.heart, len(DATA.data["sentence"]) - 1 - DATA.heart)
            for offset in range(max_offset + 1):
                indices = []
                if DATA.heart + offset < len(DATA.data["sentence"]):
                    indices.append(DATA.heart + offset)
                if offset > 0 and DATA.heart - offset >= 0:
                    indices.append(DATA.heart - offset)
                        ...
                        for iiii in range(8):
                            ...
                            if reply:
                                if (i != len(DATA.data["sentence"]) and
                                    ...
                                    print("返信: {}, {}, {}".format(DATA.data["sentence"][i + iiii + 1][0], DATA.data["sentence"][i + iiii + 1][1], i + iiii + 1))
        ...
        # Third block with relaxed conditions
        rate = 1
        while rate > 0:
            rate -= a
            max_offset = max(DATA.heart, len(DATA.data["sentence"]) - 1 - DATA.heart)
